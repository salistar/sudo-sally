...
const privacy: LegalPage = {
  ...
  blocks: [
    ...
    },
    {
      kind: 'li',
      text: L(
        'Real‑time duels & calls — 1v1 play uses a real‑time socket connection; optional voice/video calls are peer‑to‑peer (WebRTC) and use public STUN servers to connect. Call media is exchanged directly between players, not stored by us.',
        'Duels et appels en temps réel — le jeu 1v1 utilise une connexion socket en temps réel ; les appels audio/vidéo facultatifs sont pair‑à‑pair (WebRTC) et utilisent des serveurs STUN publics pour se connecter. Le flux des appels est échangé directement entre les joueurs et n’est pas stocké par nos soins.',
        'المبارزات والمكالمات الفورية — يستخدم اللعب 1 ضد 1 اتصال socket فوري؛ أما المكالمات الصوتية/المرئية الاختيارية فهي من نظير إلى نظير (WebRTC) وتستخدم خوادم STUN العامة للاتصال. يتم تبادل وسائط المكالمة مباشرة بين اللاعبين ولا نقوم بتخزينها.',
      ),
    },
